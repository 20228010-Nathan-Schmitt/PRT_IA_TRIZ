...
def get_embedding(sentences:list[str]):
    ...
    for j in range(len(sentences)):
        ...
        for i in range(text_preprocessed["input_word_ids"].shape[1]):
            ...
            word = get_vocab_by_id(id)
            wordEmbedMatch[j].append([word, embeddings[j,i].numpy()])
            word2vec[word] = embeddings[j,i].numpy()
    return wordEmbedMatch, word2vec

def cosineDistanceWord(embeddings, sentence1, index1, sentence2, index2):
...
if __name__=="__main__":
    ...
    embeddings, _ = get_embedding(text_test)
